...
- **Analytics:** GA4 or Plausible + consent banner. **Deploy:** Railway (auto-deploys from `main`), domain axonahq.com.
...
- **Always** capture a corrected mistake as a durable rule in docs/lessons.md (or the relevant agent/rule file) so it can't recur.
...
focus, keyboard, alt) · passes anti-slop · consent + privacy present. One-line standard: _"Would
this feel at home next to linear.app or harvey.ai? If not, it isn't done."_

## Compaction

When compacting/summarizing this session, always preserve: the active StoryID, the list of modified
files, the gate command for the story, and any failing-gate output. Drop prose recaps, not these.
