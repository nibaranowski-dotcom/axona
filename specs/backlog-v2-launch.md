...
| 2 | V2.2 | **SEO.1** — per-page metadata + OG image + canonical (`goaxona.com`) + Organization JSON-LD; `sitemap.ts` + `robots.ts` | P0 | M | todo | `NEXT_PUBLIC_SITE_URL=https://goaxona.com` |
...
| 9 | V2.9 | **Deploy** — Vercel project + `goaxona.com` domain + DNS; preview → production | P0 | M | todo | Grab `goaxona.com` (was free); Vercel Domains = one-click DNS |
...
Rationale: a working form + clean metadata + a11y/perf + a real URL gets you a link you can send *now*
(fabricated traction is fine while it's a private/direct share). The honest-traction + legal pass is
what's required before it's *public*.
