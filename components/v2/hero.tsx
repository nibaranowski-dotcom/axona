...
import { MonoChip, Skel } from "@/components/v2/ui";
import { HeroEmailCapture } from "@/components/v2/email-capture";
...
export function Hero() {
  return (
    <section className="mx-auto max-w-[1180px] px-7 pt-16">
      ...
      <p className="mt-[22px] max-w-[52ch] text-[clamp(17px,1.8vw,21px)] text-[--body]">
        {hero.sub}
      </p>
      ...
      <div className="dotgrid relative mt-12 flex min-h-[540px] items-center justify-center overflow-hidden rounded-[14px] bg-panel">
        <div className="relative h-[340px] w-[min(560px,80%)]">
          ...
          <div className="absolute left-0 top-12 w-[230px] rounded-[10px] border border-line2 bg-white p-4 shadow-[0_18px_40px_rgba(0,0,0,0.07)]">
            <div className="font-mono text-[9.5px] tracking-[0.08em] text-cap">
              BOM · WORK ORDER
            </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] tracking-[0.06em] text-white/55">
                UNIT · SN-2208
              </span>
            ...
            <div className="flex items-end gap-[5px]">
              {[14, 22, 30].map((h) => (
                <span
                  key={h}
                  className="w-[7px] rounded-[2px] bg-lime"
                  style={{ height: h }}
                />
              ))}
              {[24, 16].map((h) => (
                <span
                  key={h}
                  className="w-[7px] rounded-[2px] bg-white/25"
                  style={{ height: h }}
                />
              ))}
      ...
      <div className="mr-[22px] flex items-center gap-2 border-r border-line pr-[22px]">
        <span className="grid grid-cols-3 gap-[2px]" aria-hidden="true">
          {[1, 1, 0, 1, 0, 1].map((on, i) => (
            <span
              key={i}
              className={`size-[3px] ${on ? "bg-ink" : "bg-line2"}`}
            />
          ))}
        </span>
        <span className="font-mono text-[10.5px] tracking-[0.06em] text-[--body]">
          {agentStats.label}
        </span>
      </div>
