import { introAgents, keepFocused, aiLearns, scale, receipts, brand } from "@/content/site-v2";
    ...
    <section className="mx-auto max-w-[1180px] px-7 pt-5">
      <div className="grid items-stretch gap-5 md:grid-cols-2">
        <div className="py-10">
          ...
          <p className="mt-6 max-w-[40ch] text-[15px] leading-[1.5] text-[--body]">{introAgents.quote}</p>
          <div className="mt-[18px] text-[14px] text-dim">{introAgents.attribution}</div>
          <div className="mt-6">
        ...
        <div className="relative flex min-h-[340px] items-center justify-center rounded-[14px] bg-panel">
          <span className="text-[clamp(40px,5vw,70px)] font-bold tracking-[-0.04em] text-ink">{brand.name}</span>
          <span className="absolute left-4 top-[14px] font-mono text-[10px] text-cap">{introAgents.lockupTag}</span>
        </div>
      </div>
    </section>
  );
      ...
      <div className="grid items-center gap-12 md:grid-cols-2">
        ...
        >
          <div className="absolute bottom-[22px] left-[22px] text-ink">
            <div className="text-[38px] font-bold tracking-[-0.02em]">{keepFocused.portrait.name}</div>
            <div className="text-[13.5px] text-[--body]">{keepFocused.portrait.role}</div>
          </div>
          <span className="absolute left-4 top-[14px] font-mono text-[10px] text-cap">{keepFocused.portrait.tag}</span>
        </div>
        <div>
          <h2 className="max-w-[16ch] text-[clamp(26px,3.2vw,42px)] font-semibold leading-[1.06] tracking-[-0.03em] text-ink">
            {keepFocused.h2}
          </h2>
          <p className="mt-[22px] max-w-[42ch] text-[15px] leading-[1.5] text-[--body]">{keepFocused.body}</p>
...
export function AILearns() {
  return (
    <section className="mx-auto max-w-[1180px] px-7 py-12">
      <div className="grid items-center gap-12 md:grid-cols-2">
        <div>
          <h2 className="max-w-[14ch] text-[clamp(26px,3.2vw,42px)] font-semibold leading-[1.06] tracking-[-0.03em] text-ink">
            {aiLearns.h2}
          </h2>
          <p className="mt-[22px] max-w-[42ch] text-[15px] leading-[1.5] text-[--body]">{aiLearns.body}</p>
        </div>
        <div className="min-h-[300px] rounded-[14px] bg-panel p-[30px]">
          <div className="rounded-[12px] border border-line2 bg-white p-[22px]">
            <div className="mb-[18px] flex items-center justify-between">
              <span className="text-[15px] font-semibold text-ink">{aiLearns.policyTitle}</span>
  ...
  return (
    <section className="mx-auto max-w-[1180px] px-7 py-[72px] text-center">
      ...
      <div className="mt-12 grid gap-4 text-left md:grid-cols-2">
        {scale.cards.map((c) => (
          <div key={c.title} className="flex min-h-[260px] flex-col rounded-[14px] bg-panel p-7">
            ...
            <h3 className="mt-[22px] text-[18px] font-semibold tracking-[-0.02em] text-ink">{c.title}</h3>
            <p className="mt-1.5 text-[14px] leading-[1.45] text-[--body]">{c.body}</p>
          </div>
        ))}
      </div>
    </section>
...
export function Receipts() {
  return (
    <section id="customers" className="mx-auto max-w-[1180px] px-7 pb-20 pt-10 text-center">
      ...
      <div className="mt-12 grid gap-3.5 text-left sm:grid-cols-2 lg:grid-cols-4">
        {receipts.items.map((t) => (
          <div key={t.logo} className="flex min-h-[200px] flex-col rounded-[12px] border border-line p-[22px]">
            <span className="text-[13px] font-bold tracking-[-0.01em] text-logo">{t.logo}</span>
            <p className="mt-4 text-[13.5px] leading-[1.45] text-ink">“{t.quote}”</p>
